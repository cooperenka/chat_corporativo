...
def load_data():
    ...
    # Restaurar active_users desde sesiones guardadas
    # Así los usuarios siguen apareciendo online aunque el servidor se reinicie
    global active_users
    active_users = {}
        ...
        if username in users_db:
            if username not in active_users:
                active_users[username] = {
                    ...
                    'lastSeen': time.time() - 60,  # 60s atrás: "ausente" pero no offline inmediato
                    'token': token
                }
...
class ChatHandler(BaseHTTPRequestHandler):
        ...
        try:
            with data_lock:
                ...
                users_list = [
                    ...
                    if now - user['lastSeen'] < 120  # 2 minutos de tolerancia
                ]
    ...
    def _get_contacts(self):
        ...
        try:
                ...
                contacts_list = [
                    {
                        ...
                        'displayName': user['displayName'],
                        'online': (
                            user['username'] in active_users and
                            now - active_users[user['username']]['lastSeen'] < 120  # 2 minutos de tolerancia
                        )
                    }
            ...
            # Ordenar: primero online, luego alfabético
            contacts_list.sort(key=lambda u: (0 if u['online'] else 1, u['displayName'].lower()))
