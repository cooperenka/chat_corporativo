...
// Sonido de notificación - Generado con Web Audio API (no depende de archivos externos)
let _audioCtx = null;
...
function playNotificationSound() {
    try {
        ...
        // Nota 2 - confirmación (120ms después)
...
function startChatSession() {
    ...
    usersInterval = setInterval(() => { loadUsers(); loadContacts(); loadGroups(); }, 7000);
        ...
        if (data.success) {
            ...
            if (groupMessages.length > oldCount) {
                ...
                if (newFromOthers.length > 0) {
                    newFromOthers.forEach((msg, i) => {
                        setTimeout(() => {
                            playNotificationSound();
                            if (!document.hasFocus()) {
                                showNotification(
                                    `${msg.displayName} en ${selectedGroup.name}`,
                                    msg.text || '📎 Archivo',
...
async function checkUnreadMessagesForAllUsers() {
    ...
    // Verificar no-leídos tanto para activos como para todos los contactos
    const allUsersToCheck = [
        ...activeUsers,
        ...contacts.filter(c => !activeUsers.some(a => a.username === c.username))
    ].filter(
        user => !(selectedUser && selectedUser.username === user.username)
    );
...
function renderUsersList() {
    ...
    if (counter) counter.innerHTML = `🟢 ${onlineCount} en línea · 👥 ${contacts.length} contactos · 🏢 ${groups.length} grupo${groups.length !== 1 ? 's' : ''}`;
    ...
    container.innerHTML = `
        <div class="flex border-b border-gray-200 bg-white sticky top-0 z-10">
            ...
            <button onclick="switchSidebarTab('contacts')"
                ...
                👥 (${contacts.length})
                ${totalUnreadContacts > 0 && sidebarTab !== 'contacts' ? `<span class="absolute top-1 right-1 w-4 h-4 bg-red-500 rounded-full text-white text-xs flex items-center justify-center leading-none">${totalUnreadContacts > 9 ? '9+' : totalUnreadContacts}</span>` : ''}
            </button>
            <button onclick="switchSidebarTab('groups')"
                class="flex-1 py-2 text-xs font-semibold transition-colors ${sidebarTab==='groups' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700'}">
                🏢 (${groups.length})
            </button>
        </div>
        ...
        return `
            <button onclick="selectGroup('${group.id}')"
                ...
                <div class="flex items-center gap-3">
                    <div class="w-11 h-11 rounded-xl flex items-center justify-center text-white text-lg font-bold shadow-md flex-shrink-0" style="background: ${groupColor}">
                        ${group.name.charAt(0).toUpperCase()}
                    </div>
                    <div class="flex-1 min-w-0">
                        <div class="font-semibold text-sm text-gray-900 truncate">${escapeHtml(group.name)}</div>
                        <div class="text-xs text-gray-500">${group.memberCount} miembro${group.memberCount !== 1 ? 's' : ''}${group.description ? ' · ' + escapeHtml(group.description.substring(0, 25)) : ''}</div>
                    </div>
...
function renderContactsList(content) {
    ...
    let html = visible.map(user => buildUserCard(user, user.online)).join('');
        ...
        >
            <div class="flex items-center gap-3">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="text-xs truncate ${unreadCount > 0 ? 'text-blue-600 font-semibold' : isOnline ? 'text-green-600' : 'text-gray-400'}">
                        ${unreadCount > 0 
...
function selectGroup(groupId) {
    ...
    if (!group) return;
    selectedGroup = { ...group };
    selectedUser = null; // Cerrar chat privado si había uno
    groupMessages = [];
    loadGroupMessages();
    render();
}
