...
push_subscriptions = {}  # username -> [subscription_dict, ...]
data_lock = Lock()
import random, string

# ── PUSH NOTIFICATIONS (VAPID) ──
PUSH_SUBS_FILE = 'chat_push_subs.json'
VAPID_FILE = 'chat_vapid_keys.json'
_vapid_keys = None
...
try:
    ...
    print("✅ pywebpush disponible — Push notifications activas")
except ImportError:
    print("⚠️  pywebpush no instalado — Push notifications desactivadas")
    print("   Para activarlas: pip install pywebpush")

def generate_vapid_keys():
    """Genera o carga las claves VAPID para Web Push"""
    global _vapid_keys
    if os.path.exists(VAPID_FILE):
        try:
            with open(VAPID_FILE, 'r') as f:
                _vapid_keys = json.load(f)
            print(f"✅ Claves VAPID cargadas desde {VAPID_FILE}")
            return
        except Exception:
            pass
    ...
    try:
        ...
        _vapid_keys = {
            ...
            'claims': {'sub': 'mailto:admin@cooperenka.com'}
        }
        with open(VAPID_FILE, 'w') as f:
            json.dump(_vapid_keys, f, indent=2)
        print(f"✅ Claves VAPID generadas y guardadas en {VAPID_FILE}")
    except Exception as e:
        print(f"⚠️  No se pudieron generar claves VAPID: {e}")
        _vapid_keys = None

def load_push_subscriptions():
    """Cargar suscripciones push"""
    global push_subscriptions
    if os.path.exists(PUSH_SUBS_FILE):
        try:
            with open(PUSH_SUBS_FILE, 'r') as f:
                push_subscriptions = json.load(f)
            print(f"✅ Cargadas {sum(len(v) for v in push_subscriptions.values())} suscripciones push")
        except Exception:
            push_subscriptions = {}
...
def save_push_subscriptions():
    try:
        ...
        print(f"⚠️  Error guardando suscripciones push: {e}")

def send_push_notification(to_username, title, body, tag='msg', chat_type='dm', chat_id=''):
    """Enviar push notification a un usuario"""
    if not _pywebpush_available or not _vapid_keys:
        return
    
    subs = push_subscriptions.get(to_username, [])
    if not subs:
        return
    ...
    for i, sub in enumerate(subs):
        try:
            webpush(
                ...
                vapid_claims=_vapid_keys['claims']
            )
...
class ChatHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed_path.path == '/' or parsed_path.path == '/index.html':
            ...
            self._serve_sw()
        elif parsed_path.path == '/api/push/vapid-key':
            self._get_vapid_key()
    ...
    def do_POST(self):
        ...
        elif parsed_path.path == '/api/push/subscribe':
            self._push_subscribe()
        elif parsed_path.path == '/api/push/unsubscribe':
    ...
    def _serve_sw(self):
        """Servir Service Worker con scope correcto"""
        self.send_response(200)
        self.send_header('Content-type', 'application/javascript; charset=utf-8')
        self.send_header('Cache-Control', 'no-store')
    ...
    def _get_vapid_key(self):
        """Retornar la clave pública VAPID para push"""
        if _vapid_keys and _vapid_keys.get('public_key'):
            self._set_headers()
            self.wfile.write(json.dumps({'publicKey': _vapid_keys['public_key']}).encode())
        else:
            self._set_headers(status=404)
            self.wfile.write(json.dumps({'error': 'Push no configurado'}).encode())

    def _push_subscribe(self):
        """Registrar suscripción push: POST /api/push/subscribe"""
        try:
            ...
            if not subscription:
                ...
                self.wfile.write(json.dumps({'success': False, 'error': 'subscription requerida'}).encode())
                return
            ...
            with data_lock:
                ...
                # Evitar duplicados
                existing = [s.get('endpoint') for s in push_subscriptions[username]]
                if subscription.get('endpoint') not in existing:
                    push_subscriptions[username].append(subscription)
                    save_push_subscriptions()
            
            self._set_headers()
            self.wfile.write(json.dumps({'success': True}).encode())
            print(f"✅ Push subscription registrada para {username}")
    ...
    def _push_unsubscribe(self):
        """Eliminar suscripción push: POST /api/push/unsubscribe"""
        try:
            auth_header = self.headers.get('Authorization', '')
            token = auth_header.replace('Bearer ', '') if auth_header.startswith('Bearer ') else None
            if not token or token not in sessions:
            ...
            with data_lock:
                if username in push_subscriptions:
                    push_subscriptions[username] = [
                        s for s in push_subscriptions[username] if s.get('endpoint') != endpoint
                    ]
                    save_push_subscriptions()
    ...
    def _send_message(self):
        ...
        try:
            ...
            try:
                sender_name = users_db[current_username]['displayName']
                preview = text[:80] if text else '📎 Archivo adjunto'
                send_push_notification(
                    to_username, sender_name, preview,
                    tag=f'dm_{current_username}', chat_type='dm', chat_id=current_username
                )
            except Exception:
                pass
    ...
    def _send_group_message(self, parsed_path):
        ...
        try:
            ...
            # Enviar push a todos los miembros del grupo excepto el remitente
            try:
                sender_name = users_db[username]['displayName']
                preview = text[:80] if text else '📎 Archivo adjunto'
                for member in group.get('members', []):
                    if member != username:
                        send_push_notification(
                            member, f"{sender_name} · {group['name']}", preview,
                            tag=f'group_{group_id}', chat_type='group', chat_id=group_id
                        )
            except Exception:
                pass
